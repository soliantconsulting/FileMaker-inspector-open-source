...
| parseLayouts | attr:'show' | gap | catalog-portal-setup | Portal Options show bitmask, now down to one bit. Bit 2 (allow delete) is covered by `layout.contents.objects[].allowDelete`; fm 0.8.0 reports bit 8 (sorted) as `objects[].sort` and bit 16 (filtered) as `objects[].filter`. Bit 1 (allow create) still has no fm key, and is the whole of what remains here. |
...
| parseBrokenReferences | qsa:'Portal' | covered | — | Here the selector exists only to reach the portal's filter Calculation, which fm 0.8.0 reports as `layout.contents.objects[].filter`. |
...
| parseBitFlags | attr:'Options' | gap | catalog-layout-parts | Part Definition options word, now a gap for three part types rather than ten. fm 0.8.0 reports the dialog as named keys on `parts[].pagination` — `breakBefore`, `breakAfterEvery`, `restartPageNumbers`, `allowBreakAcrossPages`, `discardRemainder` — and the body's alternate and active row state as `parts[].rowState.{useAlternate,useActive}`. The members are per part type, measured on ooe: `body` and `leadingSubSummary` carry all five, the grand summaries four each, `header`/`footer`/`titleHeader` only `restartPageNumbers`, and `topNavigation`/`bottomNavigation`/`titleFooter` carry **no pagination object at all** — which is what keeps this a gap. The Bit Flags tab is retired: it catalogued raw option words because SaXML leaves them undecoded; fm reports every option as a named key, so there is nothing to decipher. |
...
| parseBitFlags | attr:'show' | gap | catalog-portal-setup | Portal 'show' bits: delete is covered by `objects[].allowDelete`, and fm 0.8.0 reports sort and filter as `objects[].sort` and `objects[].filter` - the help's "reported nowhere yet" is no longer true. Allow-create is what is left of the portal-setup gap. The Bit Flags tab is retired: it catalogued raw option words because SaXML leaves them undecoded; fm reports every option as a named key, so there is nothing to decipher. |
...
| parseUnreferenced | qsa:'Portal > Calculation' | covered | — | The portal filter calculation, reported by fm 0.8.0 as `layout.contents.objects[].filter`. It is calculation text under a key no name list claims, so ui/analysis/refs.js tokenises it: a field referenced only from a portal filter is found now, where before it was reported unreferenced. |
...
- `catalog-layout-parts` (4 rows, narrowed on 2026-09-16 and again on 2026-09-23): fm 0.7.0 reports `layout.parts[]{type,height,offset,name,breakField}`, which covers part presence, count, type, order and geometry, so the Wireframe tab and the header/footer/sub-summary/navigation layout lists have a source now. fm 0.8.0 then reported the Part Definition dialog itself, as `parts[].pagination` and `parts[].rowState` — so the options word is a gap for three part types rather than ten, `topNavigation`/`bottomNavigation`/`titleFooter` being the three fm reports no pagination object for. What is left across every part type is the raw part type code, the part definition kind, the object count per part, and any style or CSS a part wears - a part entry carries no style key at all.
...
- `catalog-portal-setup` (2 rows, narrowed on 2026-09-23): was the portal's filter calculation, its stored sort order and its allow-create bit. fm 0.8.0 reports the filter as `objects[].filter` and the sort as `objects[].sort.fields[]{field,order}`, so `portals_with_filter` and `portals_with_sort` have a source, and a field referenced only from a portal filter is no longer a false positive in the unreferenced-fields report - refs.js tokenises the filter text. What is left is the allow-create bit, which has no fm key, and six sort details fm still omits: the `blanksLast` and `maintain` switches (missing on relationship sort specs too) and the sort field's id and table-occurrence identity, the field being reported by qualified name only.
